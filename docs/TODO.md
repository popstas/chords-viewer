...
- [x] **Фаза 0 — ветка + скаффолд.** Ветка `vue3` от `master`; заменить зависимости в
...
- [x] **Фаза 1 — поднять на Nuxt 3.** Чтобы `npm run dev` (порт 3001) стартовал без ошибок в
...
- [x] **Фаза 2 — стор на Pinia (самое рискованное).** `store/index.js` → Pinia-стор
...
- [x] **Фаза 3 — порт остальных компонентов.** Иконки `<icon>` →
...
- [x] **Фаза 4 — ленивая подгрузка списка на мобиле (главная сложность).** `SongList.vue` на
...
- [x] **Фаза 5 — Playwright smoke E2E + проверка паритета.** Добавить `@playwright/test`,
