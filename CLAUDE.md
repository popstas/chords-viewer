...
- **Chord nowrap mode** — `chordNowrap` is a *transient* store flag (**not** in `persist.pick`) toggled from a button left of the beat toggle in `SongItem.vue`. The button is **always rendered** for an active song (active state painted `.song-transpose__nowrap_active` → `color: var(--border-hover) !important` + `svg { fill }`; the icon path is `fill="currentColor"`, and Element's compiled theme needs the `!important` to win). **`chordNowrap` is derived per song open, not reset in the store:** `changeSong`/`setPrev`/`setNext` deliberately do NOT touch it (that previously raced the auto-apply). The sole writer on open is `SongItem.measureChordsOverflow`, which measures `scrollWidth > clientWidth` client-only by force-`nowrap` (mount / song-open / font-change / debounced resize — never during SSR) and, once per open on a *reliable* (laid-out, `clientWidth > 0`) reading, sets `chordNowrap = overflow` (ON when chord lines overflow, OFF otherwise). A once-per-open `nowrapAutoApplied` guard (re-armed in the `active` watcher) makes a later resize/font re-measure never override a manual toggle. On → `.song-item__line_chords_nowrap` halves the inter-chord gaps (each `&nbsp;` is wrapped in `.song-item__chord-space`, shrunk via `font-size: 0.5em` — `word-spacing` can't, browsers don't apply it to U+00A0), then `overflow-x: auto` horizontal scroll handles any still-too-long line.
